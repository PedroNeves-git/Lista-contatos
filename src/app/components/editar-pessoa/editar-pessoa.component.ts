...
export class EditarPessoaComponent implements AfterViewInit {
  id?: number;
  pessoa?: Pessoa;
  ...
  ngOnInit() {
    this.id = this.route.snapshot.params['id'];  // Captura o id da URL
  ...
  atualizarCadastro() {
    if (!this.id) return;  // Garante que id esteja definido
    ...
    this.pessoaService.atualizarPessoa(this.id!, pessoa).subscribe({
      next: () => {
        ...
        this.abrirModal();
        this.router.navigate(['/']);  // Redireciona após sucesso
      },
